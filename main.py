...
try:
    if WEB_APP_URL:
        bot.set_chat_menu_button(menu_button=types.MenuButtonWebApp(type="web_app", text="Apri Rai News", web_app=types.WebAppInfo(url=WEB_APP_URL)))
...
def open_button():
    if WEB_APP_URL:
        return types.InlineKeyboardButton(text="📰 Apri Rai News", web_app=types.WebAppInfo(url=WEB_APP_URL))
    return types.InlineKeyboardButton(text="📰 Apri Rai News", url="https://www.rainews.it")
...
def start(message):
    ...
    text = "📰 *Benvenuti su Rai News.*\n\n_«L'informazione è un diritto di tutti.»_\n\nDal *1954*, la Rai accompagna gli italiani con informazione, cultura e intrattenimento. Oggi anche qui su Telegram. Ogni stagione una piccola selezione di cultura, viaggi, cucina, scienza e sport, da leggere in chat con calma.\n\nPer cominciare, toccate *I titoli del giorno*."
